...
function SpyMasterCardView({ card }: { card: GameCard }) {
  return (
    <><ModalContent hidden={card.guessed}>
      <Box boxSize='sm' borderWidth='1px' borderRadius='lg' overflow='hidden' borderColor="gray" color={card.color}>
        <Heading as='h4'>
          {card.name}
        </Heading>
      </Box>
    </ModalContent><ModalContent hidden={!card.guessed}>
        <Box boxSize='sm' borderWidth='1px' borderRadius='lg' overflow='hidden' color={card.color} borderColor={card.color}>
          <Heading as='h4'>
            {card.name}
          </Heading>
        </Box>
...
function OperativeCardView({ card }: { card: GameCard }) {
  return (
    <><ModalContent hidden={card.guessed}>
      <Button boxSize='sm' borderWidth='1px' borderRadius='lg' overflow='hidden' color="gray" name={card.name}>
        <Heading as='h4'>
          {card.name}
        </Heading>
        onClick={async () => {
        ...
        <Button boxSize='sm' borderWidth='1px' borderRadius='lg' overflow='hidden' color={card.color} name={card.name} disabled={card.guessed}>
          <Heading as='h4'>
            {card.name}
          </Heading>
        </Button>
